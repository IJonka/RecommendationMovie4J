...
async def add_user(user_id, user_name, user_date):
    ...
    if len(list(exists)) == 0:
        q = """CREATE (user:USER {user_id: r_1, user_name: 'r_2', user_date: 'r_3'})""".replace(
            "r_1", str(user_id)).replace(
            "r_2", str(user_name)).replace(
            "r_3", str(user_date))
...
async def get_movie_search(text, amount=1):
    ans = conn.query(f"""MATCH (movie:MOVIE)
                        WHERE toLower(movie.name) contains '{text}' OR movie.date = '{text}' 
                        OR toLower(movie.ganre) contains '{text}' OR
